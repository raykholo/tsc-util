...
function isCenter(point: GridPoint, cols: number, rows: number): boolean {
  ...
  return point.row === Math.floor(rows / 2) &&
         point.col === Math.floor(cols / 2);
}
...
function createArrayGrid(
  ...
  for (const row of rowOrder) {
    for (const col of colOrder) {
      ...
      points.push({ x, y, index: index, gridIndex: index, row, col, totalRows: rows, totalCols: cols });
      index++;
    }
  }
...
export const ArrayGrid: React.FC<ArrayGridProps> = ({
  ...
  let points = createArrayGrid(cols, rows, spacingX, spacingY, startX, startY, startCorner, anchor, startIndex);
  ...
  const getComponentForPoint = (point: GridPoint): React.ReactElement | null => {
    ...
    <>
      {points.map((point) => {
        ...
        if (replacementComponent) {
          ...
          const newName = existingName ? `${existingName}${point.index}` : `grid_item_${point.index}`;

          return (
            <group key={point.index} pcbX={point.x} pcbY={point.y}>
              {React.cloneElement(replacementComponent as React.ReactElement<any>, {
